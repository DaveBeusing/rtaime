...
AP-16 verifies both durability lanes before activating recovered authority. A valid latest checkpoint restores the exact persisted Production Revision and routing, after which Runtime execution is queried and reconciled. Runtime at the same committed revision is rebound as-is; Runtime behind/idle is reapplied at the same authoritative revision; Runtime ahead of durable Control authority is a fail-closed recovery conflict.
