...
The Program monitor shows `ON AIR` only while the existing local Clean Program Output controller reports `OUTPUT LIVE`. External transmission is not inferred from this state.
