...
public sealed class LocalMediaTransportIntegrationTests
{
	...
	public void Multi_frame_mp4_supports_play_pause_seek_step_stop_and_av_positioning()
	{
		...
		var path = MaterializeTransportAsset();
		try
		{
			...
			var open = provider.TryOpen(path, mediaSourceId, assetId);
			...
			Assert.True(afterSeek.Succeeded, afterSeek.Failure?.Message);
	...
	private static string MaterializeTransportAsset()
	{
		var encodedPath = Path.Combine(
			...
			"TestAssets",
			"media",
			"reference-1080p50-h264-aac-80ms.mp4.b64");
		var path = Path.Combine(Path.GetTempPath(), $"rtaime-transport-{Guid.NewGuid():N}.mp4");
		File.WriteAllBytes(path, Convert.FromBase64String(File.ReadAllText(encodedPath).Trim()));
		return path;
	}
