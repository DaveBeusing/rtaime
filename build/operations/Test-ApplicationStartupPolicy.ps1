...
Assert-Condition ($appHostReferences[0].Groups[1].Value -match 'rtaime\.Core\\rtaime\.Core\.csproj
...
) "AppHost may reference only dependency-neutral rtaime.Core; service hosts and production implementations remain forbidden."
Assert-Condition ($solution -match 'src/Hosts/rtaime\.AppHost/rtaime\.AppHost\.csproj') "Primary solution must contain rtaime.AppHost."
