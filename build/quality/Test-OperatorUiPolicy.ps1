...
Assert-Condition ($outputHealthViewModel -match '"cpu",[\s\S]+Unavailable,[\s\S]+"UNVERIFIED"' -and $outputHealthViewModel -match '"memory",[\s\S]+Unavailable,[\s\S]+"UNVERIFIED"' -and $outputHealthViewModel -match '"gpu",[\s\S]+_control\.GpuUtilization') "Unavailable CPU/Memory must remain unverified while GPU uses Runtime-published evidence."
