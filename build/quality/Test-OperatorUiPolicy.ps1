...
$rundownPath = Join-Path $repositoryRoot "src/Hosts/rtaime.Operator/RundownControl.xaml"
...
$monitorPresentationControlPath = Join-Path $repositoryRoot "src/Hosts/rtaime.Operator/Controls/RtaimeMonitorPresentation.cs"
...
$rundown = Get-Content -LiteralPath $rundownPath -Raw
$rundownViewModel = Get-Content -LiteralPath $rundownViewModelPath -Raw
...
$monitorPresentationControl = Get-Content -LiteralPath $monitorPresentationControlPath -Raw
...
Assert-Condition ($monitorPresentationControl -match 'class RtaimeMonitorPresentation : ContentControl' -and $monitorPresentationControl -match 'MonitorProperty' -and $monitorPresentationControl -match 'RoleStatusProperty') "Preview and Program must share an explicit monitor-presentation control contract."
Assert-Condition ($monitorWorkspaceTheme -match 'x:Key="RtaimeMonitorPresentation"' -and $monitorWorkspaceTheme -match 'Monitor\.Frame' -and $monitorWorkspaceTheme -match 'Monitor\.DisplayTimecode') "Shared monitor presentation must own the common frame, overlay and timecode chrome."
Assert-Condition ($previewViewer -match '<controls:RtaimeMonitorPresentation' -and $previewViewer -match 'RoleText="PREVIEW"' -and $previewViewer -match 'RtaimeStatusKind\.Preview') "Preview viewer must compose the shared monitor presentation with Preview semantics."
Assert-Condition ($programViewer -match '<controls:RtaimeMonitorPresentation' -and $programViewer -match 'RoleText="PROGRAM"' -and $programViewer -match 'RtaimeStatusKind\.Program') "Program viewer must compose the shared monitor presentation with Program semantics."
Assert-Condition ($monitorWorkspaceTheme -match 'RtaimeMonitorPresentation[\s\S]+RtaimeMonitorHeader' -and $monitorWorkspaceTheme -match 'RtaimeMonitorPresentation[\s\S]+RtaimeMonitorTransport') "The shared monitor presentation must own the 40px header and 42px transport chrome."
...
Assert-Condition ($previewViewer -notmatch '<Image\s' -and $programViewer -notmatch '<Image\s' -and $monitorWorkspaceTheme -match '<Image Source="\{Binding Monitor\.Frame') "Preview and Program frame rendering must be centralized in the shared monitor presentation."
Assert-Condition ($previewViewerCode -match 'PreviewViewer : MonitorView' -and $programViewerCode -match 'ProgramViewer : MonitorView') "Preview and Program must retain separate role controls over the shared MonitorView contract."
...
Assert-Condition ($monitorWorkspaceTheme -match 'Monitor\.ShowSafeArea' -and $monitorWorkspaceTheme -match 'Monitor\.ShowCenterMark' -and $monitorWorkspaceTheme -match 'Monitor\.ShowGrid') "Shared monitor presentation must own Safe Area, Center Mark and Grid overlays."
Assert-Condition ($monitorWorkspaceTheme -match 'Monitor\.DisplayTimecode' -and $monitorView -match 'IsTransportSource') "Production monitor timecode must be shown only when the loaded transport source matches the viewed source."
Assert-Condition ($previewViewer -match 'PlayPauseCommand' -and $previewViewer -match 'StopCommand' -and $previewViewer -match 'SetInCommand' -and $previewViewer -match 'SetOutCommand' -and $previewViewer -match 'PreviousCueCommand' -and $previewViewer -match 'NextCueCommand') "Preview must retain Media Deck transport, IN/OUT and cue interaction."
Assert-Condition ($programViewer -notmatch 'PlayPauseCommand|StopCommand|SetInCommand|SetOutCommand|JumpInCommand|JumpOutCommand|PreviousCueCommand|NextCueCommand') "Program must never receive Preview transport commands."
Assert-Condition ($programViewer -match 'Content="ON AIR"' -and $programViewer -match 'OutputState' -and $programViewer -match 'CommitStatus' -and $programViewer -match 'TransitionStatus') "Program must retain its role-specific live, output, commit and transition presentation."
...
Assert-Condition ($window -match '<local:RundownControl' -and $window -match '<local:MediaTimelineControl') "The persistent lower workspace must host the governed Rundown beside the existing timeline."
Assert-Condition ($rundown -match 'ItemsSource="{Binding Items}"' -and $rundown -match 'Command="{Binding PrepareCommand}"' -and $rundown -match 'Command="{Binding GoCommand}"' -and $rundown -match 'Command="{Binding NextCommand}"' -and $rundown -match 'Command="{Binding PreviousCommand}"') "Rundown must expose explicit local selection and governed prepare/GO/navigation controls."
Assert-Condition ($rundown -match 'controls:RtaimeButton' -and $rundown -match 'controls:RtaimeTextBox' -and $rundown -match 'controls:RtaimeCheckBox' -and $rundown -match 'controls:RtaimeListBox') "Rundown surface must use the custom rtaime interaction controls."
Assert-Condition ($rundown -notmatch '<(Button|ToggleButton|CheckBox|RadioButton|TextBox|ComboBox|ListBox|ListView|DataGrid|Slider|ScrollBar|ScrollViewer)(\s|/|>)') "Rundown must not reintroduce directly visible stock WPF interactive chrome."
Assert-Condition ($rundownViewModel -match '_client\.PrepareRundownItemAsync' -and $rundownViewModel -match '_client\.GoRundownAsync' -and $rundownViewModel -match '_client\.SaveRundownAsync') "Rundown Operator mutations must cross the Client SDK seam."
Assert-Condition ($rundownViewModel -notmatch 'RuntimeHost|Provider\.' -and $rundownViewModel -notmatch 'NamedPipeRuntime') "Rundown presentation must not bypass ControlHost through Runtime/provider dependencies."
Assert-Condition ($rundownViewModel -match 'RundownAdvanceMode\.AutoOnMediaEnd' -and $rundownViewModel -match 'RundownTransition\.Dissolve' -and $rundownViewModel -match 'RundownTransition\.Cut') "Rundown authoring must expose only the governed bounded auto-advance and CUT/DISSOLVE semantics."
Assert-Condition ($rundownDocumentation -match 'ControlHost' -and $rundownDocumentation -match 'does not introduce a second Program authority' -and $rundownDocumentation -match 'No UI timer advances production') "Rundown documentation must preserve authority, recovery and deterministic completion boundaries."
...
Assert-Condition ($window -match 'Binding ApplyTextGraphicsCommand' -and $window -match 'Binding GraphicsText' -and $window -match 'Binding GraphicsTypeface' -and $window -match 'Binding GraphicsFallbackTypeface' -and $window -match 'Binding GraphicsFontSize') "Graphics workflow must expose the governed Production CG lower-third editor."
Assert-Condition ($viewModel -match '_client\.ApplyProductionCgTextAsync') "Production CG text must cross the Client SDK seam instead of rendering in Operator."
Assert-Condition ($viewModel -match 'CanApplyGraphicsPlacement[\s\S]+ProductionCgText\.Active[\s\S]+bitmap-graphics') "Bitmap placement must remain available for the confirmed bitmap layer while Production CG keeps definition-owned placement."
Assert-Condition ($mediaPool -match 'IsGraphicsSelection[\s\S]+layer:bitmap-graphics') "Selecting the confirmed bitmap compositing layer must expose the existing graphics transform Inspector."
Assert-Condition ($viewModel -notmatch 'FormattedText|DrawingContext|RenderTargetBitmap') "Operator must not render authoritative Production CG text locally."
...
Assert-Condition ($window -match 'Binding Startup\.Stages' -and $window -match 'Startup\.ActiveStageName' -and $window -match 'Startup\.Summary' -and $window -match 'Startup\.NextStep') "Startup presentation must expose the AppHost lifecycle stage projection, active-stage detail and next safe operator step."
Assert-Condition ($window -match 'Startup\.StartupVisualState' -and $theme -match 'OperatorStartupStateText' -and $theme -match 'Value="DEGRADED"' -and $theme -match 'Value="FAILED"') "Startup presentation must distinguish degraded and failed startup states with semantic text and chrome."
...
Assert-Condition ($window -match 'Startup\.ToggleDetailsCommand' -and $window -match 'Startup\.EvidencePath' -and $window -match 'Startup\.DiagnosticPath') "Startup presentation must provide collapsible technical lifecycle and diagnostic evidence."
Assert-Condition ($window -match 'Startup\.CopyDiagnosticsCommand' -and $window -match 'Startup\.OpenDiagnosticsCommand' -and $startupViewModel -match 'BuildDiagnosticsText' -and $startupViewModel -match 'ResolveDiagnosticOpenTarget') "Startup diagnostics actions must remain local, evidence-driven and presentation-only."
Assert-Condition ($window -match 'FocusManager\.FocusedElement="\{Binding ElementName=StartupDetailsButton\}"' -and $window -match 'KeyboardNavigation\.TabNavigation="Cycle"' -and $window -match 'AutomationProperties\.Name="Toggle startup technical details"') "Startup failure presentation must keep keyboard focus on the visible recovery surface and expose technical details accessibly."
Assert-Condition ($startupViewModel -match '_latchedFailureStageId' -and $startupViewModel -match 'UpdateFailureLatch' -and $startupViewModel -match 'recoveryStage\.Status, "READY"') "Startup failure presentation must latch until the failed stage publishes explicit Ready recovery."
Assert-Condition ($startupViewModel -notmatch 'RetryCommand|RestartCommand') "Operator startup must not expose retry or restart commands without a safe lifecycle contract."
...
Assert-Condition ($stateControlTheme -match 'RtaimeLoadingIndicator' -and $stateControlTheme -notmatch '<Storyboard|<DoubleAnimation|<ColorAnimation|<ThicknessAnimation' -and $stateControlTheme -match 'OperatorUiStateKind\.Recovering' -and $stateControlTheme -match 'OperatorUiStateKind\.Ready') "State control theme must provide semantic loading/recovery presentation without decorative animation and automatically collapse Ready states."
...
Assert-Condition ($mediaPool -match 'ImportMediaAssetsAsync' -and $mediaPool -match 'MediaAssetCatalogSnapshot' -and $windowCode -match 'PickLocalMediaFiles') "Media Pool import must use the persistent media asset catalogue and bounded multi-file picker."
Assert-Condition ($window -match 'OnMediaPoolAddClick' -and $window -match 'Header="GENERATOR"' -and $window -match 'Header="Test Signal"' -and $windowCode -match 'OnMediaPoolTestSignalMenuClick') "Media Library Add must expose the internal Test Signal generator without bypassing the catalogue import path."
Assert-Condition ($window -match 'Header="Relink Media' -and $window -match 'Header="Remove from Library"' -and $windowCode -match 'OnMediaPoolRelinkActionClick' -and $windowCode -match 'OnMediaPoolRemoveActionClick') "Media Library must expose explicit relink and remove-from-library workflows."
Assert-Condition ($mediaPool -match 'AddTestSignalCommand' -and $mediaPool -match 'SetStaticTestSignalCommand' -and $mediaPool -match 'SetMotionTestSignalCommand' -and $mediaPool -match 'SetAvSyncTestSignalCommand' -and $mediaPool -match 'DisableTestSignalCommand') "Media Pool must expose explicit static, motion, A/V sync and off test-signal actions."
Assert-Condition ($inspectorHost -match 'Content="TEST SIGNAL"' -and $inspectorHost -match 'Content="STATIC"' -and $inspectorHost -match 'Content="MOTION"' -and $inspectorHost -match 'Content="A/V SYNC"' -and $inspectorHost -match 'Content="OFF"') "Inspector must expose the governed test-signal presets through custom rtaime controls."
Assert-Condition ($viewModel -match 'ConfigureTestSignalAsync' -and $viewModel -match 'SetBroadcastTestPatternAsync' -and $viewModel -match 'SetAudioTestSignalAsync' -and $viewModel -match 'OperatorTestSignalPreset\.AvSync') "Operator test-signal presets must reuse the existing video and generated-audio Client SDK seams."
Assert-Condition ($mediaPool -match 'TestSignalResolutionLabel' -and $mediaPool -match 'TestSignalFrameRateLabel' -and $mediaPool -match 'TestSignalTimecodeEnabled' -and $mediaPool -match 'TestSignalFrameCounterEnabled') "Test Signal Inspector must project effective format and motion diagnostics instead of inventing independent format controls."
...
Assert-Condition ($mediaPool -match 'SupportsTransformRotation => HasAuthoritativeCompositingSelection' -and $mediaPool -match 'SupportsTransformAnchor => HasAuthoritativeCompositingSelection' -and $mediaPool -match 'SupportsTransformCrop => HasAuthoritativeCompositingSelection' -and $inspectorHost -match 'Binding Inspector\.SelectedLayerRotationDegrees' -and $inspectorHost -match 'Binding Inspector\.SelectedLayerAnchorX' -and $inspectorHost -match 'Binding Inspector\.SelectedLayerAnchorY' -and $inspectorHost -match 'Binding Inspector\.SelectedLayerCropLeft' -and $inspectorHost -match 'Binding Inspector\.SelectedLayerCropTop' -and $inspectorHost -match 'Binding Inspector\.SelectedLayerCropRight' -and $inspectorHost -match 'Binding Inspector\.SelectedLayerCropBottom' -and $inspectorHost -match 'Binding Inspector\.ApplyCompositingTransformCommand') "Authoritative transform capabilities must be selection-gated and expose Rotation, Anchor and Crop through the governed Inspector command path."
Assert-Condition ($mediaPool -match 'ApplyColorGradeCommand' -and $mediaPool -match 'RemoveProcessingNodeCommand' -and $inspectorHost -match 'Binding Inspector\.SelectedColorGradeEnabled' -and $inspectorHost -match 'Binding Inspector\.SelectedColorGradeBrightness' -and $inspectorHost -match 'Binding Inspector\.SelectedColorGradeContrast' -and $inspectorHost -match 'Binding Inspector\.SelectedColorGradeSaturation' -and $inspectorHost -match 'Binding Inspector\.ApplyColorGradeCommand' -and $inspectorHost -match 'Binding Inspector\.RemoveProcessingNodeCommand') "Bounded Color Grade controls must remain selection-driven and reuse the governed Inspector command path."
...
Assert-Condition ($shell -match 'CompactNavigationWidth = 1320' -and $shell -match '\? 68 : 92' -and $shell -match 'LeftSplitterWidth.+: 6' -and $shell -match 'RightSplitterWidth.+: 6') "Mockup shell must retain the 92px reference navigation rail, compact to 68px below the responsive breakpoint and keep 6px horizontal gutters."
...
Assert-Condition ($shell -match 'CompactNavigationWidth = 1320' -and $shell -match 'NavigationRailWidth' -and $shell -match 'Visibility\.Collapsed' -and $shell -match 'UpdateViewportSize' -and $shell -match 'WorkspaceScale' -and $shell -match 'value\.Value / WorkspaceScale') "Production shell must adapt navigation and persisted splitter geometry to the current view container while preserving the 1920x1080 reference layout."
...
Assert-Condition ($outputHealthViewModel -match 'new OutputStatusViewModel\("program", "PROGRAM"\)' -and $outputHealthViewModel -match 'new OutputStatusViewModel\("preview", "PREVIEW"\)' -and $outputHealthViewModel -match 'new OutputStatusViewModel\("aux", "AUX"\)' -and $outputHealthViewModel -match 'new OutputStatusViewModel\("clean-program", "CLEAN FEED"\)') "Production output projection must expose Program, Preview, governed Aux and local Clean Feed roles without conflating monitoring with physical output authority."
Assert-Condition ($outputHealthViewModel -match '_control\.OutputRoles\.FirstOrDefault' -and $outputHealthViewModel -match 'aux\.HealthState' -and $outputHealthViewModel -match 'aux\.Evidence' -and $outputHealthViewModel -match 'Aux output role is not configured by authoritative Control state') "Aux output must project confirmed backend evidence and remain fail-closed when unavailable."
...
Assert-Condition ($tokens -match '<Color x:Key="OperatorColorDiagnosticSurface">#0A141C</Color>' -and $tokens -match '<Color x:Key="OperatorColorGraphSurface">#13222C</Color>' -and $tokens -match '<Color x:Key="OperatorColorGraphSurfaceRaised">#1B303C</Color>') "Compositing graph semantic tokens must retain the exact #0A141C / #13222C / #1B303C mockup palette."
...
Assert-Condition ($compositingGraph -notmatch 'DropShadowEffect|BlurRadius|ShadowDepth') "Compositing nodes must remain flat and must not introduce large shadow effects."
...
Assert-Condition ($compositingGraphViewModel -match 'RoleLabel' -and $compositingGraphViewModel -match '"MEDIA INPUT"' -and $compositingGraphViewModel -match '"OUTPUT ROUTER"' -and $compositingGraphViewModel -match '"TRANSFORM"' -and $compositingGraphViewModel -match '"PROCESSING"' -and $compositingGraphViewModel -match '"MERGE"' -and $compositingGraphViewModel -match '"RECORDER"') "Compositing role labels must map only current projected data into mockup roles, including authoritative Transform and Processing nodes."
...
Assert-Condition ($compositingGraph -match 'CONFIRMED LAYER STACK' -and $compositingGraph -notmatch 'Content="REWIRE"') "Source topology rewiring must remain unavailable while the governed Runtime layer stack is explicitly identified as confirmed."
Assert-Condition ($compositingGraph -match 'MoveSelectedLayerDownCommand' -and $compositingGraph -match 'MoveSelectedLayerUpCommand' -and $compositingGraph -match 'ToggleSelectedLayerCommand' -and $compositingGraph -match 'DecreaseSelectedLayerOpacityCommand' -and $compositingGraph -match 'IncreaseSelectedLayerOpacityCommand') "Compositing layer controls must expose bounded order, visibility and opacity mutations through custom controls."
...
Assert-Condition ($compositingGraphViewModel -match 'LayerCommandStatus = "APPLYING"' -and $compositingGraphViewModel -match '_operator\.SetCompositingLayerStateAsync' -and $compositingGraphViewModel -match '_operator\.ReorderCompositingLayersAsync') "Compositing layer mutations must remain client-mediated and visibly pending until confirmed state returns."
...
Assert-Condition ($compositingWorkspaceSurface -match '<local:PreviewViewer' -and $compositingWorkspaceSurface -match 'Monitoring\.PreviewImage' -and $compositingWorkspaceSurface -match 'MediaDeck\.TogglePlayPauseCommand' -and $previewViewer -match '<controls:RtaimeMonitorPresentation' -and $monitorWorkspaceTheme -match 'x:Key="RtaimeMonitorPresentation"') "Compositing Preview must reuse the shared monitor presentation and existing monitoring/transport paths."
Assert-Condition ($compositingWorkspaceSurface -match 'Shell\.CompositingPreviewHeight' -and $shell -match 'CompositingPreviewHeight => Math\.Max\(220, 390 \* WorkspaceScale\)' -and $compositingWorkspaceSurface -match '<RowDefinition Height="6" />' -and $compositingWorkspaceSurface -match 'SYSTEM &amp; PERFORMANCE') "Compositing right stack must retain the 390px reference Preview height responsively, above a 6px gap and compact System & Performance surface."
...
Assert-Condition ($liveSceneCue -match 'Binding DisplayIndex' -and $liveSceneCue -match 'DesiredStateSummary' -and $liveSceneCue -match 'ProgramSource\.Remaining' -and $liveSceneCue -match 'Content="NEXT"' -and $liveSceneCue -match 'Content="LIVE"' -and $liveSceneCue -match 'RtaimeIconMoreGeometry') "LIVE scene rows must expose index, declared desired Scene state, duration, NEXT/LIVE states and own overflow affordance."
Assert-Condition ($liveSceneCue -match 'SelectedItem="\{Binding SelectedScene, Mode=TwoWay\}"' -and $liveSceneCue -match 'Command="\{Binding ActivateSceneCommand\}"' -and $liveSceneCue -match 'Command="\{Binding CutCommand\}"' -and $liveSceneCue -notmatch 'SelectionChanged=') "LIVE Scene selection must remain presentation-only while activation is an explicit command and the legacy CUT path remains available."
Assert-Condition ($viewModel -match 'Scenes = new ObservableCollection<OperatorSceneViewModel>' -and $viewModel -match 'ActivateSceneAsync' -and $viewModel -match 'ActiveSceneName' -and $viewModel -match 'SceneFailureReason') "Operator Scene presentation must expose explicit activation and confirmed Scene evidence."
...
Assert-Condition ($liveSceneCue -notmatch 'NOT AVAILABLE IN V1 CONTRACT' -and $liveSceneCue -match 'ACTIVE SCENE' -and $liveSceneCue -match 'SceneFailureReason') "LIVE Scene controls must expose governed activation evidence instead of the former unavailable placeholder."
...
Assert-Condition ($liveControls -match 'EngineLifecycleState' -and $liveControls -match 'EngineHealth' -and $liveControls -match 'RuntimeHealth' -and $liveControls -match 'MediaHealth' -and $liveControls -match 'Notifications\.ActiveProblems' -and $liveControls -match 'controls:RtaimeAlertRow') "LIVE alerts must retain shared lifecycle/health rows and project persistent errors through the centralized notification surface."
