...
Assert-Condition ($outputHealthViewModel -match 'ResolveRenderStatus' -and $outputHealthViewModel -match 'render <= 3\.0' -and $window -match 'Engineering target: ≤3 ms' -and $window -match 'P95 ceiling: 5 ms') "Operator render latency must retain the explicit 3 ms engineering target and 5 ms hardware qualification ceiling."
