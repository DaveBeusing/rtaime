...
Assert-Condition ($programViewer -match 'Value="OUTPUT LIVE"' -and $programViewer -match 'Value="ON AIR"' -and $programViewer -match 'External transmission remains separately unverified') "Program ON AIR presentation must derive from the existing local Program Output state without claiming external transmission."
