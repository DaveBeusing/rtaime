...
Assert-Condition ($outputHealthViewModel -match 'ResolveRenderStatus' -and $outputHealthViewModel -match 'render < 5\.0' -and $window -match 'Production target: below 5 ms') "Operator render latency must retain the explicit sub-5 ms production target and warning state."
