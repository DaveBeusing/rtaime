...
$featureXamlFiles = @(Get-ChildItem -LiteralPath $operatorXamlRoot -Filter "*.xaml" -File -Recurse | Where-Object {
	$relativePath = [System.IO.Path]::GetRelativePath($operatorXamlRoot, $_.FullName).Replace("\", "/")
...
foreach ($featureFile in $featureXamlFiles) {
	$relativePath = [System.IO.Path]::GetRelativePath($operatorXamlRoot, $featureFile.FullName).Replace("\", "/")
	try {
	...
	foreach ($node in $featureDocument.SelectNodes("//*")) {
		...
		if ($node.LocalName -eq "Border" -and $node.HasAttribute("CornerRadius")) {
			...
			if ($radiusValue -match '^\d+(?:\.\d+)?(?:,\d+(?:\.\d+)?){0,3}$topBarStart = $window.IndexOf('x:Name="TopBar"', [StringComparison]::Ordinal)
