...
Assert-Condition ($window -match 'OnMediaPoolAddClick' -and $window -match 'Header="GENERATOR"' -and $window -match 'Header="Test Signal"' -and $windowCode -match 'OnMediaPoolTestSignalMenuClick') "Media Library Add must expose the internal Test Signal generator without bypassing the existing import path."
