...
Assert-Condition ($shell -match 'CompositingGraphVisibility' -and $shell -match 'StandardViewerVisibility => IsLiveWorkspace \|\| IsCompositingWorkspace \|\| IsEditWorkspace' -and $shell -match 'NonLiveCenterVisibility => IsLiveWorkspace \|\| IsCompositingWorkspace') "COMPOSITING and EDIT must use dedicated center surfaces instead of duplicating the standard monitor stack."
...
Assert-Condition ($customControls -match 'class RtaimeMetricRing' -and $customControls -match 'protected override void OnRender' -and $customControls -match 'HasValueProperty') "Compositing performance rings must be own presentation controls and render unavailable metrics without a fabricated value arc."
...
Assert-Condition ($outputHealthViewModel -match '"cpu",[\s\S]{0,220}Unavailable,[\s\S]{0,120}"UNVERIFIED"' -and $outputHealthViewModel -match '"memory",[\s\S]{0,220}Unavailable,[\s\S]{0,120}"UNVERIFIED"' -and $outputHealthViewModel -match 'TryParsePercentage\(_control\.GpuUtilization\)' -and $outputHealthViewModel -match 'NormalizeAvailability\(_control\.GpuUtilization\)') "Unavailable CPU/Memory must remain unverified while GPU uses Runtime-published evidence."
Assert-Condition ($compositingWorkspaceSurface -match 'SYSTEM &amp; PERFORMANCE' -and $compositingWorkspaceSurface -match 'OutputHealth\.GpuMetric\.HistoryPoints' -and $compositingWorkspaceSurface -match 'OutputHealth\.VramMetric\.Value' -and $compositingWorkspaceSurface -match 'OutputHealth\.RenderMetric\.Value') "Compositing performance must show engine status, real metric cards and a bounded real-sample history graph."
...
Assert-Condition ($multiview -match 'Binding IsPreview' -and $multiview -match 'Binding IsProgram' -and $multiview -match 'Content="PGM"' -and $multiview -match 'Content="PVW"') "LIVE source tallies must derive from authoritative PGM/PVW routing."
...
Assert-Condition ($outputHealthControl -match 'Height="270"' -and $outputHealthControl -match 'Property="Height" Value="48"' -and $outputHealthControl -match 'Binding AssignedSource' -and $outputHealthControl -match 'Binding Status' -and $outputHealthControl -match 'Binding FrameRate') "LIVE Output Routing must retain compact table-like four-row density with assigned source and status/fps."
...
Assert-Condition ($liveControls -match 'EngineLifecycleState' -and $liveControls -match 'EngineHealth' -and $liveControls -match 'RuntimeHealth' -and $liveControls -match 'MediaHealth' -and $liveControls -match 'LastError' -and $liveControls -match '<Ellipse') "LIVE alerts must project existing lifecycle/health/error evidence with severity-dot presentation."
