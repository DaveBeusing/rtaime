...
Assert-Condition ($runtime -match 'snapshot\.Performance') "AP-54 RuntimeHost support snapshots must reuse the live performance snapshot."
Assert-Condition ($runtime -match 'runtime\.droppedFrames') "AP-54 support diagnostics must include the bounded dropped-frame counter."
...
Assert-Condition ($runtimeService -match 'V1RuntimePerformanceSnapshot') "RuntimeHost must expose a bounded AP-54 performance snapshot."
Assert-Condition ($runtimeService -match 'GpuUtilizationPercent[\s\S]*GpuVramUsedBytes') "AP-54 performance telemetry must keep optional GPU utilization and VRAM fields explicit."
Assert-Condition ($frameDrop -match 'class RuntimeFrameDropCounter') "AP-54 must retain a dedicated O(1) dropped-frame counter."
...
Assert-Condition ($healthProjection -match 'Pass[\s\S]*Fail[\s\S]*Unverified') "AP-54 health projection must preserve PASS/FAIL/UNVERIFIED semantics."
Assert-Condition ($healthProjection -match 'GpuUtilizationPercent is') "AP-54 must expose GPU utilization only when a measured value exists."
...
Assert-Condition ($documentation -match 'AP-54 Runtime health') "Observability documentation must record the AP-54 health/performance projection."
...
Write-Host "AP-54 health telemetry: bounded frame metrics and explicit GPU UNVERIFIED evidence"
