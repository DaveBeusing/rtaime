...
switch ([string]$binding.qualificationType) {
	...
	"TIMING_REFERENCE_SOAK" {
		...
		Assert-Condition ([uint64]$payload.statistics.CaptureFailures -eq 0) "Timing qualification recorded capture failures during soak."
		Assert-Condition ([uint64]$payload.statistics.OutputRejected -eq 0) "Timing qualification recorded hard Program-output failures during soak."
		Assert-Condition ([uint64]$payload.statistics.CapturedA -ge [uint64]$payload.minimumContinuityFrames) "Timing qualification did not sustain input A cadence."
		Assert-Condition ([uint64]$payload.statistics.CapturedB -ge [uint64]$payload.minimumContinuityFrames) "Timing qualification did not sustain input B cadence."
		Assert-Condition ([uint64]$payload.statistics.OutputAccepted -ge [uint64]$payload.minimumContinuityFrames) "Timing qualification did not sustain Program-output cadence."
		Assert-Condition ([int]$payload.hostCycle.sampleCount -ge 10) "Timing qualification has too few host-cycle samples."
		Assert-Condition ([double]$payload.hostCycle.p95Milliseconds -le [double]$payload.hostCycle.maximumAllowedP95Milliseconds) "Timing qualification host-cycle p95 exceeds its declared bound."
		Assert-Condition ([string]$payload.telemetry.status -eq "PASSED") "Timing qualification lacks PASSED hardware telemetry continuity evidence."
		Assert-Condition ([int]$payload.telemetry.sampleCount -ge 30) "Timing qualification has too few hardware telemetry samples."
		Assert-Condition ([int]$payload.telemetry.completeSampleCount -eq [int]$payload.telemetry.sampleCount) "Timing qualification contains incomplete hardware telemetry samples."
		Assert-Condition (-not [string]::IsNullOrWhiteSpace([string]$payload.telemetry.gpuDriverVersion)) "Timing qualification lacks NVIDIA driver identity."
		...
		Assert-Condition ([string]$payload.audioVideoSynchronization.status -eq "PASSED") "Timing qualification lacks PASSED physical A/V synchronization evidence."
		Assert-Condition ([int]$payload.audioVideoSynchronization.sampleCount -ge 30) "Timing qualification has too few physical A/V synchronization samples."
	}
	default {
